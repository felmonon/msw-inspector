...
__export(index_exports, {
  ...
  createRecordId: () => createRecordId,
  createRegExpPattern: () => createRegExpPattern,
  createUnknownPattern: () => createUnknownPattern,
  formatCoverageReport: () => formatCoverageReport,
  normalizeLocation: () => normalizeLocation,
  normalizeMethod: () => normalizeMethod,
  normalizePathname: () => normalizePathname,
  scanApiCalls: () => scanApiCalls,
...
function matchesPath(call, handler) {
  ...
  for (const handlerCandidate of handlerCandidates) {
    ...
    for (const callCandidate of callCandidates) {
      const { origin, path: path2 } = splitOriginAndPath(callCandidate);
...
function createUnknownPattern(raw) {
  return {
    raw,
    kind: "unknown",
    normalized: raw.trim(),
...
function resolveUrlExpression(node, context) {
  ...
  if (import_ts_morph.Node.isNewExpression(node) && isIdentifierName(node.getExpression(), "URL")) {
    ...
    const path2 = resolveString(pathArg, context);
    if (!path2) {
      return null;
    }
    const base = baseArg ? resolveString(baseArg, context) : null;
    try {
      return base ? new URL(path2, base).toString() : new URL(path2).toString();
    } catch {
      return null;
    }
  }
...
function resolveUrlExpression2(node, context) {
  ...
  if (import_ts_morph2.Node.isNewExpression(node) && isIdentifierName2(node.getExpression(), "URL")) {
    const [pathArg, baseArg] = node.getArguments();
    const path2 = pathArg ? resolveStaticString(pathArg, context) : null;
    const base = baseArg ? resolveStaticString(baseArg, context) : null;
    if (path2 === null || base === null) {
      return null;
    }
    try {
